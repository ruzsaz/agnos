...
function Panel(panelInitString, mediator, isLegendRequired, leftOffset, rightOffset, topOffset, bottomOffset) {
    ...
    var rearrangePanels = function() {
        ...
        var selfIndex = global.positionInArrayByProperty(panels, "id", that.panelId.substring(1));
        ...
        if (flipIndex > -1) {
            var rowDist = Math.floor(selfIndex / global.panelNumberOnScreen) - Math.floor(flipIndex / global.panelNumberOnScreen);
            var colDist = (selfIndex % global.panelNumberOnScreen) - (flipIndex % global.panelNumberOnScreen);
...
Panel.prototype.killPanel = function(panelId) {
    if (panelId === undefined || panelId === this.panelId) {
        ...
        this.panelDiv.styles(global.getStyleForScale(1, centerX, centerY))
                .style("opacity", "1")
                .transition().duration(global.selfDuration)
                .styles(global.getStyleForScale(0, centerX, centerY))
                .style("opacity", "0")
                .remove()
